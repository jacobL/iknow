# -*- coding: utf-8 -*-
"""
Created on Fri Mar 19 11:00:30 2021
...
def Iknow_QS(QS_Json,Stand_QS_dict) :
    query_corpus = QS_iKnow_model.Query_corpus(QS_Json["user_question"])
    pred_category = QS_iKnow_model.GetQuerySystemAnswer(query_corpus)
    ErrorCode , answerType , QS_AnsNO = QS_iKnow_model.GetQuerySimilarityAnswer_monpa(pred_category,QS_Json["user_question"])
...
def QS_feedback(QS_FB) :
    ...
    conn  =  pymysql.connect ( host = '10.55.23.101' ,port=35092 ,  user = 'root' ,  passwd = "1234" , db='qs') 
    cur  =  conn.cursor ()
...
#前端呼叫後端    
@app.route('/I_Know_QS', methods = ['GET'])
def I_Know_QS():
    try:
