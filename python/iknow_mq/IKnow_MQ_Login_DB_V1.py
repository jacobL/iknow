...
def CheckAD(UserID,password):
    ...
    # 2020/04/9 IT因病毒影響許多server，此連結失效
...
def login():  
    ...
    #conn  =  pymysql.connect ( host = '10.55.23.101' ,port=33060 ,  user = 'root' ,  passwd = "1234" , db='mq')
    conn  =  pymysql.connect ( host = '10.55.23.168' ,port=33060 ,  user = 'root' ,  passwd = "1234" , db='mq')
    cur = conn.cursor();
    errMsg = ''
    ...
    if errMsg == '' : # 登入成功
        ...
        # 20200309 會發生replace(ID,'')錯誤，造成英中文一起 yujie.chen陳郁潔
        #userChineseName = json_data1['displayname'][0].replace(ID,'').replace(' ','') 
        userChineseName = json_data1['displayname'][0].split(' ')[1]
...
def IKnow():
    if current_user.is_active: 
        ...
        if cur.rowcount > 0 :
            #print(UserID,' 取得mq_user_loader資料')
            r = cur.fetchone()
            employeeid = r[0]
            userChineseName = r[1]
            OU = r[2]
            ...
            #print(UserID,' 無法取得mq_user_loader資料')
...
def mq_associstion(conn,answerNo):
    ...
    cur  =  conn.cursor ()
    sql_dateno="Select distinct DateNo from mq.mq_association_rule Order By DateNo desc"
    cur.execute(sql_dateno)
    result = cur.fetchone()
    dateno=str(result[0])
    sql="Select Question_Y from mq.mq_association_rule Where Question_X='" + answerNo + "' \
...
def MQ_Insert_log(conn,Answer_Json) :
    ...
    select_sql="SELECT * FROM mq.mq_chat_log_new Where chat_id='" + Chat_ID + "'"
    #conn  =  pymysql.connect ( host = '10.56.211.124' ,port=3306 ,  user = 'testuser' ,  passwd = "1688"  ) 
    cur  =  conn.cursor ()
    ...
    if e is None:
        if Answer_Json['answerType']=="standard" :
            log_sql="INSERT INTO mq.mq_chat_log_new (chat_id,session_id,user_id,user_name,checktime,chat,answerType \
        ...
        log_sql="UPDATE mq.mq_chat_log_new SET answerNO='" + Answer_Json['data'][0]['answerNo'] + "' \
...
def MQ_feedback(Q_FB) :
    ...
    update_sql="UPDATE mq.mq_chat_log_new SET feedback='" + feedback + "',suggest='" + suggest + "' \
        Where chat_id = '" + Chat_ID + "'"
    #conn  =  pymysql.connect ( host = '10.55.23.101' ,port=33060 ,  user = 'root' ,  passwd = "1234" , db='mq')
    conn  =  pymysql.connect ( host = '10.55.23.168' ,port=33060 ,  user = 'root' ,  passwd = "1234" , db='mq')
    cur  =  conn.cursor ()
...
def Select_TopQ(Q_System) :
    ...
    MQ_original_question['All']=["AERB 涵蓋類型有哪些","AERB TAT的規範是什麼","如何查詢DCC文件","台灣撥打其他Site短號","如何查詢生產良率"]
    MQ_original_question['glossary']=["什麼是EOL","什麼是ND Filter","什麼是API","什麼是8D","什麼是ECRS","什麼是ECRSAA"]
    MQ_original_question['abnormal_handing']=["AERB 涵蓋類型有哪些","AERB TAT的規範是什麼","什麼是CAERB的風險度評分(RPN)","CAERB結案時，如何確認是否需啟動QBQ會議","IMP 系統如何進入"]
    MQ_original_question['quality_system']=["如何查詢DCC文件","如何查詢VLRR","什麼是VLRR","白血球倉別KY2為何","什麼是白血球"]
    MQ_original_question['specification_judge']=["何謂IIS","動態規格調整是包含什麼","Mura應該在什麼畫面下判定","什麼是紅燈右轉","何謂表面能(達因值)測試"]
    MQ_original_question['process_production']=["如何查詢生產良率","什麼是CT1/CT2","什麼是C檢","LCD 製程主要有哪些","何謂LCM 製程"]
    MQ_original_question['quality_activity']=["台灣撥打其他Site短號","如何預約電話會議","如何加入電話會議","何謂CRN ","如何補打卡"]
    ...
    conn  =  pymysql.connect ( host = '10.55.23.168' ,port=33060 ,  user = 'root' ,  passwd = "1234" , db='mq')
    cur  =  conn.cursor ()
    sql_dateno="Select distinct DateNo from mq.mq_top_question Where System_NO='" + System + "' \
    ...
    for c in category_sub:
        sql="Select Question from mq.mq_top_question Where DateNO='" + dateno + "' \
...
if __name__ == '__main__':    
    ...
    #app.run(host='10.56.244.27', port=8100)
    #hpcip = '0.0.0.0' 
