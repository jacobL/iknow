...
def Load_Jiebadict(conn):
    ...
    SQL_Str='SELECT jiebaword FROM mq.mq_jiebadict'
    ...
    #同義字
    SQL_Str='SELECT WORD,SYNONYMOUS FROM mq.mq_jieba_synon'
    dbCursor.execute(SQL_Str)
    results = dbCursor.fetchall()
    dict_Name=[]
    dict_Value=[]
    ...
    SQL_Str='SELECT stopword FROM mq.mq_stopword'
    dbCursor.execute(SQL_Str)
    results = dbCursor.fetchall()
    stop_words=[]
    for db_row in results:
...
def Build_BOW(conn,bc,stop_words,tongyici_dict):
    #標準問法題庫
    query="SELECT * FROM mq.mq_standand_question Where Enabled='True'"   
...
#     print(Stand_A_dict)
    ...
    #相似問題題庫
    query='SELECT * FROM mq.mq_sim_question'  
...
def Build_Association(conn):
    query = "select * from mq.mq_chat_log_new where answerNo is not null order by id desc"
    ...
    for suggest in suggest_rules:
        ...
        Lift=suggest[4]
        log_sql="INSERT INTO mq.mq_association_rule (DateNO,Question_X,Question_Y,Support,Confidence,Lift) \
            VALUES ('" + now_time + "','" + Q1 + "','"  + Q2 + "','" + Support + "','" + Confidence + "','" + Lift + "')"
#         log_sql="INSERT INTO mq.mq_association_rule VALUES ('" + now_time + "','" + Q1 + "', \
...
def Build_Top_Question(conn):
    ...
    query = "SELECT mq.mq_chat_log_new.answerNO,mq.mq_standand_question.category_main,\
    mq.mq_standand_question.category_sub,mq.mq_standand_question.Question \
    ,COUNT(mq.mq_chat_log_new.id) AS amount FROM mq.mq_chat_log_new inner join \
    mq.mq_standand_question on mq.mq_chat_log_new.answerNo=mq.mq_standand_question.Answer_NO \
    WHERE mq.mq_chat_log_new.checktime>='" + start_time + "' AND \
    mq.mq_chat_log_new.checktime<='" + end_time + "' AND \
    mq.mq_chat_log_new.answerNo IS NOT NULL AND mq.mq_chat_log_new.answerNo<>'' AND \
    mq.mq_chat_log_new.answerNo<>'AEXXXX' AND mq.mq_standand_question.system='MQ' \
    GROUP BY mq.mq_chat_log_new.answerNO ORDER BY amount DESC Limit 5"
    User_log_df = pd.read_sql(query, conn)
    cur  =  conn.cursor ()
    for i in range(0,len(User_log_df)):
        insert_sql="INSERT INTO mq.mq_top_question (DateNO,System_NO,category_sub,Answer_NO,Question,amount) \
    ...
    for c in category_sub:
        ...
        WHERE mq.mq_chat_log_new.checktime>='" + start_time + "' AND \
        mq.mq_chat_log_new.checktime<='" + end_time + "' AND \
        mq.mq_standand_question.category_sub='" + c + "' AND \
        mq.mq_chat_log_new.answerNo IS NOT NULL AND mq.mq_chat_log_new.answerNo<>'' AND \
        mq.mq_chat_log_new.answerNo<>'AEXXXX' AND mq.mq_standand_question.system='MQ' \
        GROUP BY mq.mq_chat_log_new.answerNO ORDER BY amount DESC Limit 5"
        User_log_df = pd.read_sql(query, conn)
