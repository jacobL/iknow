...
def Load_Jiebadict(conn):
    ...
    SQL_Str='SELECT jiebaword FROM sqe.sqe_jiebadict'
    ...
    #同義字
    SQL_Str='SELECT WORD,SYNONYMOUS FROM sqe.sqe_jieba_synon'
    dbCursor.execute(SQL_Str)
    results = dbCursor.fetchall()
    dict_Name=[]
    dict_Value=[]
    ...
    SQL_Str='SELECT stopword FROM sqe.sqe_stopword'
    dbCursor.execute(SQL_Str)
    results = dbCursor.fetchall()
    stop_words=[]
    for db_row in results:
...
def Build_BOW(conn,bc,stop_words,tongyici_dict):
    #標準問法題庫
    query="SELECT * FROM sqe.sqe_standand_question Where Enabled='True'"   
...
#     print(Stand_A_dict)
    ...
    #相似問題題庫
    query='SELECT * FROM sqe.sqe_sim_question'  
...
def Build_Association(conn):
    query = "select * from sqe.sqe_chat_log_new where answerNo is not null order by id desc"
    ...
    for suggest in suggest_rules:
        ...
        Lift=suggest[4]
        log_sql="INSERT INTO sqe.sqe_association_rule (DateNO,Question_X,Question_Y,Support,Confidence,Lift) \
            VALUES ('" + now_time + "','" + Q1 + "','"  + Q2 + "','" + Support + "','" + Confidence + "','" + Lift + "')"
#         log_sql="INSERT INTO sqe.sqe_association_rule VALUES ('" + now_time + "','" + Q1 + "', \
...
def Build_Top_Question(conn):
    ...
    query = "SELECT sqe.sqe_chat_log_new.answerNO,sqe.sqe_standand_question.category_main,\
    sqe.sqe_standand_question.category_sub,sqe.sqe_standand_question.Question \
    ,COUNT(sqe.sqe_chat_log_new.id) AS amount FROM sqe.sqe_chat_log_new inner join \
    sqe.sqe_standand_question on sqe.sqe_chat_log_new.answerNo=sqe.sqe_standand_question.Answer_NO \
    WHERE sqe.sqe_chat_log_new.checktime>='" + start_time + "' AND \
    sqe.sqe_chat_log_new.checktime<='" + end_time + "' AND \
    sqe.sqe_chat_log_new.answerNo IS NOT NULL AND sqe.sqe_chat_log_new.answerNo<>'' AND \
    sqe.sqe_chat_log_new.answerNo<>'AEXXXX' AND sqe.sqe_standand_question.system='SQE' \
    GROUP BY sqe.sqe_chat_log_new.answerNO ORDER BY amount DESC Limit 5"
    User_log_df = pd.read_sql(query, conn)
    cur  =  conn.cursor ()
    for i in range(0,len(User_log_df)):
        insert_sql="INSERT INTO sqe.sqe_top_question (DateNO,System_NO,category_sub,Answer_NO,Question,amount) \
            VALUES ('" + DateNO + "','SQE','All','"  + str(User_log_df.iloc[i,0]) + "'\
    ...
    for c in category_sub:
        ...
        WHERE sqe.sqe_chat_log_new.checktime>='" + start_time + "' AND \
        sqe.sqe_chat_log_new.checktime<='" + end_time + "' AND \
        sqe.sqe_standand_question.category_sub='" + c + "' AND \
        sqe.sqe_chat_log_new.answerNo IS NOT NULL AND sqe.sqe_chat_log_new.answerNo<>'' AND \
        sqe.sqe_chat_log_new.answerNo<>'AEXXXX' AND sqe.sqe_standand_question.system='SQE' \
        GROUP BY sqe.sqe_chat_log_new.answerNO ORDER BY amount DESC Limit 5"
        User_log_df = pd.read_sql(query, conn)
        for i in range(0,len(User_log_df)):
            insert_sql="INSERT INTO sqe.sqe_top_question (DateNO,System_NO,category_sub,Answer_NO,Question,amount) \
                VALUES ('" + DateNO + "','SQE','" + str(User_log_df.iloc[i,2]) +"','"  + str(User_log_df.iloc[i,0]) + "'\
                ,'" + str(User_log_df.iloc[i,3]) + "'\
                ,'" + str(User_log_df.iloc[i,4]) +  "')"
            cur.execute(insert_sql)
            cur.execute("COMMIT")
